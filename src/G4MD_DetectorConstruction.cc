...
void G4MD_DetectorConstruction::DefineParameters()
{
  ...
  branch.GetChild("tyvekThickness").GetData(tyvekThickness);
  fTyvekThickness = tyvekThickness*m;
...
G4VPhysicalVolume* G4MD_DetectorConstruction::Construct()
{
  ...
  cout <<" 88888 World x y z = " << fExpVol_x << " " << fExpVol_y << " " << fExpVol_z << endl;
  ...
  // Add the soil
  // ------------
  G4double soil_x = fExpVolX;
  G4double soil_y = fExpVolY;
  G4double soil_z = fSoilThickness/2. + fTankHalfHeight + fTankThickness + fTyvekThickness; 
  ...
  // -----------------------
  G4Tubs* top_solid = new G4Tubs("top_solid", 0., fTankRadius + fTyvekThickness + fTankThickness, fTankThickness/2.,0.*deg, 360.*deg);
  G4Tubs* side_solid = new G4Tubs("side_solid", fTankRadius + fTyvekThickness, fTankRadius + fTyvekThickness + fTankThickness, fTankHalfHeight,0.*deg, 360.*deg);
  
  G4LogicalVolume* top_log = new G4LogicalVolume(top_solid, Concrete, "top_log", 0, 0, 0);
  G4LogicalVolume* bottom_log = new G4LogicalVolume(top_solid, Concrete, "bottom_log", 0, 0, 0);
  G4LogicalVolume* side_log = new G4LogicalVolume(side_solid, Concrete, "side_log", 0, 0, 0);
  
  x2 = 0.;
  y2 = 0.;
  z2 = - soil_z + (0.1 + fTankThickness*3./2.+ fTyvekThickness*2 + fTankHalfHeight*2);
  G4VPhysicalVolume* top_phys = new G4PVPlacement(0, G4ThreeVector(x2,y2,z2), top_log, "top", soil_log, false, 0);
  
  x2 = 0.;
  y2 = 0.;
  z2 = - soil_z + (0.1 + fTankThickness/2.);
  G4VPhysicalVolume* bottom_phys = new G4PVPlacement(0, G4ThreeVector(x2,y2,z2), bottom_log, "bottom", soil_log, false, 0);
  
  x2 = 0.;
  y2 = 0.;
  z2 = - soil_z + (0.1 + fTankThickness + fTyvekThickness + fTankHalfHeight);
  G4VPhysicalVolume* side_phys = new G4PVPlacement(0, G4ThreeVector(x2,y2,z2), side_log, "side", soil_log, false, 0);

  
  // Add the Tyvek (HDPE)
  // --------------------
  G4Tubs* top_tyvek_solid = new G4Tubs("top_tyvek_solid", 0., fTankRadius + fTyvekThickness, fTyvekThickness/2.,0.*deg, 360.*deg);
  G4Tubs* side_tyvek_solid = new G4Tubs("side_tyvek_solid", fTankRadius, fTankRadius + fTyvekThickness, fTankHalfHeight,0.*deg, 360.*deg);
  
  G4LogicalVolume* top_tyvek_log = new G4LogicalVolume(top_tyvek_solid, HDPE, "top_tyvek_log", 0, 0, 0);
  G4LogicalVolume* bottom_tyvek_log = new G4LogicalVolume(top_tyvek_solid, HDPE, "bottom_tyvek_log", 0, 0, 0);
  G4LogicalVolume* side_tyvek_log = new G4LogicalVolume(side_tyvek_solid, HDPE, "side_tyvek_log", 0, 0, 0);
  
  x2 = 0.;
  y2 = 0.;
  z2 = - soil_z + (0.1 + fTankThickness + fTyvekThickness*3./2 + fTankHalfHeight*2);
  G4VPhysicalVolume* top_tyvek_phys = new G4PVPlacement(0, G4ThreeVector(x2,y2,z2), top_tyvek_log, "top_tyvek", soil_log, false, 0);
  
  x2 = 0.;
  y2 = 0.;
  z2 = - soil_z + (0.1 + fTankThickness + fTyvekThickness/2.);
  G4VPhysicalVolume* bottom_tyvek_phys = new G4PVPlacement(0, G4ThreeVector(x2,y2,z2), bottom_tyvek_log, "bottom_tyvek", soil_log, false, 0);
  
  x2 = 0.;
  y2 = 0.;
  z2 = - soil_z + (0.1 + fTankThickness + fTyvekThickness + fTankHalfHeight);
  G4VPhysicalVolume* side_tyvek_phys = new G4PVPlacement(0, G4ThreeVector(x2,y2,z2), side_tyvek_log, "side_tyvek", soil_log, false, 0);
  ...
  x2 = 0.;
  y2 = 0.;
  z2 = - soil_z + (fTankThickness + fTankHalfHeight + 0.1);
  ...
  // Optical surface properties for the liner and water of the tank
  // --------------------------------------------------------------
  G4OpticalSurface * OpLinerSurface = new G4OpticalSurface("WallSurface");
  
  G4LogicalBorderSurface* topSurface 
    = new G4LogicalBorderSurface("topSurface", tank_phys, top_tyvek_phys, OpLinerSurface);
  
  G4LogicalBorderSurface* bottomSurface 
    = new G4LogicalBorderSurface("bottomSurface", tank_phys, bottom_tyvek_phys, OpLinerSurface);
  
  G4LogicalBorderSurface* sideSurface 
    = new G4LogicalBorderSurface("sideSurface", tank_phys, side_tyvek_phys, OpLinerSurface);
  ...
  G4VisAttributes* TankBoxVisAtt = new G4VisAttributes(G4Colour(0.5,0.5,0.5)); // (0.5 = grey) --> Concrete (Structure of thank) 
  TankBoxVisAtt->SetVisibility(true);
  top_log->SetVisAttributes(TankBoxVisAtt); 
  bottom_log->SetVisAttributes(TankBoxVisAtt);
  side_log->SetVisAttributes(TankBoxVisAtt);

  G4VisAttributes* TyvekBoxVisAtt = new G4VisAttributes(G4Colour(1.,0.,0.)); // (1,0,0 = Red) --> Tyvek 
  TyvekBoxVisAtt->SetVisibility(true);
  top_tyvek_log->SetVisAttributes(TyvekBoxVisAtt); 
  bottom_tyvek_log->SetVisAttributes(TyvekBoxVisAtt);
  side_tyvek_log->SetVisAttributes(TyvekBoxVisAtt);
  
  G4VisAttributes* WaterBoxVisAtt = new G4VisAttributes(G4Colour(0.,0.,1.)); // (0,0,1 = Blue) --> Water
  WaterBoxVisAtt->SetVisibility(true);
  tank_log->SetVisAttributes(WaterBoxVisAtt);

  
  G4VisAttributes* PMTBoxVisAtt = new G4VisAttributes(G4Colour(0.,0.,0.)); // (0,1,0 = Green) --> PMT
  PMTBoxVisAtt->SetVisibility(true);
  pmt_log->SetVisAttributes(PMTBoxVisAtt); 
  pmt_inside_log->SetVisAttributes(PMTBoxVisAtt);
